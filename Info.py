...
def preprocessing(desease):
    ...
    desease['저혈압'] = desease['수축기 혈압'].apply(lambda x: 1 if x <= 90 else 0)
...
def find_hospital(special_m, lati, long):
    ...
    key = "T69WAY2JScR34QOb1au5dyGVkI2pysCtAqBbXViBCCsPY2CeEGYlLvE1OVQipsXE2I%2FYIk6f8U%2Fu8nka2gXwtA%3D%3D"
    ...
    solution_df = pd.merge(solution_df,emrRealtime[['hpid', 'hvec', 'hvoc']],on='hpid',how='outer')
    ...
    bin = [-np.inf,10, 30, 60,np.inf]
    label = ['불가', '혼잡', '보통', '원활']
    solution_df['응급실포화도'] =  pd.cut(solution_df['응급실가용율'].astype(float)*100,bins=bin, labels=label,right=False)
    ...
    if special_m == "중증 아님":
        ...
        distance_df = solution_df.copy()[condition1]
    else:
        condition1 = (solution_df[special_m]=='Y') & (solution_df['가용수술실수']>0)

        condition2 = (solution_df['응급실포화도']>'불가')

        distance_df = solution_df.copy()[condition1 & condition2]
    ...
    distance_df['거리구분'] = pd.cut(distance_df['거리'].astype(float), bins= bin, labels=label)
...
with tab1:
    ...
    with col123:
        age = st.number_input('',min_value=0, max_value=100, value=0, step=1,label_visibility='collapsed')
    with col124:
        st.info('성별')
    with col125:
   ...
   ##-------------------------------------------------------------------------------------
    ...
    with col131 :
        fever = st.number_input('',min_value=0.0, max_value=100.0, value=36.50, step=0.1,label_visibility='collapsed')
    ...
    with col133 :
        location = st.text_input('','부산시 연제구 중앙대로', label_visibility='collapsed')
    ...
    with col141 :
        high_blood= st.slider('',10,200,120, label_visibility='collapsed')
    with col142 :
        st.info('이완기 혈압')
    with col143 :
        low_blood = st.slider('',10,200,80, label_visibility='collapsed')
    ...
    col160, col161, col162, col163= st.columns([0.1, 0.3, 0.1, 0.3])
    ...
    with col161 :
        special_yn= st.selectbox('',('중증 질환 선택','중증 질환 예측','중증 아님'), label_visibility='collapsed')
    ...
    col170, col171, col172, col173= st.columns([0.04, 0.9,0.1, 0.1])# col 나누기
    ...
    with col171 :
        if special_yn == "중증 질환 예측":
            ...
            patient_data = {
                "체온": [int(fever)],
                "수축기 혈압": [int(high_blood)],
                "이완기 혈압": [int(low_blood)],
    ...
    with st.form(key='tab1_first'):
        ...
        if st.form_submit_button(label ='병원조회') :
            ...
            hospital_list.sort_values(by='거리',ascending=True,inplace=True)
            hospital_list.reset_index(drop=True, inplace=True)
            #### 필요 병원 정보 추출 
            display_column = ['병원명','주소','응급연락처','응급실수','수술실수','가용응급실수','가용수술실수','응급실포화도','거리','거리구분']
            display_df = hospital_list[display_column]
            display_df.sort_values(by='거리',ascending=True,inplace=True)
            ...
            with st.expander("인근 병원 리스트", expanded=True):
                ...
                html= """<!DOCTYPE html>
                    <html>
                        <table style="height: 126px; width: 330px;"> <tbody > <tr>
                            ...
                            <td style="width: 230px;background-color: #C5DCE7;">{}</ td>""".format(name)+"""</tr>
                            <tr><td style="background-color: #2A799C;">
                            <div style="color: #ffffff;text-align: center;"> 나이 </div></td>
                            <td style="width: 230px;background-color: #C5DCE7;">{}</ td>""".format(age)+"""</tr>
                            <td style="background-color: #2A799C;">
                            <div style="color: #ffffff;text-align: center;"> 증상 </div></td>
                            <td style="width: 230px;background-color: #C5DCE7;">{}</ td>""".format(special_m)+"""</tr>
                            <td style="background-color: #2A799C;">
                            <div style="color: #ffffff;text-align: center;"> 위치 </div></td>
                            <td style="width: 230px;background-color: #C5DCE7;">{}</ td>""".format(location)+"""</tr>
                            <td style="background-color: #2A799C;">
                            <div style="color: #ffffff;text-align: center;"> 가까운 병원 </div></td>
                            <td style="width: 230px;background-color: #C5DCE7;">{}</ td>""".format(list(display_df['병원명'])[0])+"""</tr>
                        </tbody > </table> </html> """
                ...
                folium.Marker(location =[lati, long],
                            popup = popup_text , tooltip = '환자위치', icon=icon).add_to(m)
                ...
                for idx , row in hospital_list[:5].iterrows() :
                    ...
                    <html>
                        <table style="height: 126px; width: 330px;"> <tbody > <tr>
                            ...
                            <td style="background-color: #2A799C;">
                            <div style="color: #ffffff;text-align: center;"> 포화도 </div></td>
                    ...
                    if cnt == 0 :
                        folium.Marker(location =[row['위도'], row['경도']], popup = popup_text , tooltip = row['병원명'], icon=folium.Icon('red', icon='star')).add_to(m)
                        folium.PolyLine(locations=[[lati, long],[row['위도'], row['경도']]],color='red').add_to(m)
                        cnt +=1
                    else:
                        if list(hospital_list['응급실포화도'])[cnt] == '혼잡' :
                            folium.Marker(location =[row['위도'], row['경도']], popup = popup_text , tooltip = row['병원명'], icon=folium.Icon('darkpurple', icon='flag')).add_to(m)
                        else :
                            folium.Marker(location =[row['위도'], row['경도']], popup = popup_text , tooltip = row['병원명'], icon=folium.Icon('darkblue', icon='flag')).add_to(m)
                        cnt +=1
    ...
    with st.form(key='tab1_first2'):
        ...
        ## 완료시간 시간표시 cols 구성
        col180, col181 = st.columns([0.5,0.5]) # col 나누기
        with col180 :
            st.success('완료 시간')
        with col181 :
            end_time = st.time_input('',value=now_date,label_visibility='collapsed')
...
with tab2 :
    ...
    with col211:
        slider_date = st.slider('날짜 구간을 선택하세요',
                            ...
                            max_value=last_date,
                            value=(datetime.datetime(2023,1,1),datetime.datetime(2023,7,31)),format='YY-MM-DD')
    ...
    slider_week = (slider_date[0],slider_date[1])
    ...
    slider_month = (slider_date[0],slider_date[1])
    ...
    if select_bins == '일자별' :
        st.bar_chart(day_list_df['datetime'].value_counts())
        
    elif select_bins == '주별' :
        st.bar_chart(week_list_df['주별'] .value_counts())
        
    else :
        st.bar_chart(month_list_df['월별'] .value_counts())
    ...
    st.success("이송시간 통계")
    select_bins = st.radio("평균", ('일자별', '주별', '월별'), horizontal=True)
    if select_bins == '일자별' :
        group_day_time = day_list_df.groupby('datetime',as_index=False)['이송 시간'].mean()
        st.bar_chart(x = 'datetime', y ='이송 시간', data=group_day_time,use_container_width=True)
        
    elif select_bins == '주별' :
        group_week_time = week_list_df.groupby('주별')['이송 시간'].mean().reset_index()
        st.bar_chart(x = '주별', y ='이송 시간', data=group_week_time,use_container_width=True)
        
    else :
        group_month_time = month_list_df.groupby('월별')['이송 시간'].mean().reset_index()
        st.bar_chart(x = '월별', y ='이송 시간', data=group_month_time,use_container_width=True)
...
# ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ [도전 미션] ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ 
    ...
    with col240: # 일간 통계
        ...
        group_day_disease = day_list_df.groupby(['datetime','중증질환'],as_index=True)['중증질환'].count()
        group_day_disease = group_day_disease.to_frame().rename(columns = {'중증질환' : '병명'}).reset_index(drop=False)
        fig = px.pie(group_day_disease, values='병명',names='중증질환',hole=.3)
    ...
    with col241: # 일간 통계
        ...
        group_week_disease = week_list_df.groupby(['주별','중증질환'],as_index=True)['중증질환'].count()
        group_week_disease = group_week_disease.to_frame().rename(columns = {'중증질환' : '병명'}).reset_index(drop=False)
        fig = px.pie(group_week_disease, values='병명',names='중증질환',hole=.3)
        fig.update_traces(textposition='inside',textinfo='percent+label+value')
        fig.update_layout(height=400, width=400)
        fig.update_layout(font=dict(size=16))
        # fig.update(layout_showlegend=False)  # 범례 표시 제거
    ...
    with col242: # 일간 통계
        ...
        group_month_disease = month_list_df.groupby(['월별','중증질환'],as_index=True)['중증질환'].count()
        group_month_disease = group_month_disease.to_frame().rename(columns = {'중증질환' : '병명'}).reset_index(drop=False)
        fig = px.pie(group_month_disease, values='병명',names='중증질환',hole=.3)
        fig.update_traces(textposition='inside',textinfo='percent+label+value')
        fig.update_layout(height=400, width=400)
        fig.update_layout(font=dict(size=16))
        # fig.update(layout_showlegend=False)  # 범례 표시 제거
